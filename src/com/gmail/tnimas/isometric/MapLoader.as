		...
		public function makeWorld(size:Number):IsoWorld
		{
			...
			for(var i:int = 0; i < _grid.length; i++)
			{
				...
				for(var j:int = 0; j < _grid[i].length; j++)
				{
					...
					var b:Boolean = cell.walkable == "true";
					tile.walkable = b;
					tile.x = i * size;
					tile.z = j * size;
					//trace("i "+i+" j "+j);
					if ( b ) { //walkable показывает проходимость, но добавление идет от объекта\плитки, --- заменить
							
							world.addToFloor(tile); }
					else {
						 	
							world.addToWorld(tile,false); }
				}
			}
			world.Installed(i,j);
			return world;
